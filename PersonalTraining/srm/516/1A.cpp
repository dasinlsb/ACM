/*
对于每种加密后的串算出对应每个原串可能的key，设加密后的串有m个，原串有n个，相当于m个大小为n的集合求交
*/
//#line 2 "NetworkXOneTimePad.cpp"  
#include<bits/stdc++.h>
using namespace std;  
#define pb push_back  
...
class NetworkXOneTimePad {
	...
	int crack(vector <string> plaintexts, vector <string> ciphertexts)  {  
		// $CARETPOSITION$ 
    vector<string>&p=plaintexts;
    vector<string>&c=ciphertexts;
    int n=SZ(p[0]);
    map_str.clear();
    for(int i=0;i<SZ(c);i++){
      for(int j=0;j<SZ(p);j++){
        for(int k=0;k<n;k++){
          str[k]='0'+((p[j][k]-'0')^(c[i][k]-'0'));
        }
        map_str[string(str,str+n)]++;
      }
    }
    int ans=0;
    for(auto it:map_str){
