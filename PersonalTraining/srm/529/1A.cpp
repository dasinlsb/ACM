...
class KingSort{
    ...
    for(string&king:kings){
      int p=0;
      for(;p<king.size()&&king[p]!=' ';p++);
      string name=king.substr(0,p);
      item.push_back(make_pair(name,Num[king.substr(p+1)]));
    }
    sort(item.begin(),item.end());
    vector<string>ans;
    for(pair<string,int>&x:item){
      ans.push_back(x.first+" "+que[x.second-1]);
    }
    return ans;
	}
