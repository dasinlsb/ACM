...
class AlienAndHamburgers{
	...
	int getNumber(vector <int> type, vector <int> taste){
		// $CARETPOSITION$
    vector<vector<int> >item(101);
    for(int i=0;i<taste.size();i++){
      item[type[i]].push_back(taste[i]);
    }
    int Y=0,A=0;
    vector<int>wait;
    for(int i=1;i<=100;i++)if(item[i].size()){
      sort(item[i].begin(),item[i].end());
      if(item[i].back()>=0){
        Y++;
        for(;item[i].size() && item[i].back()>=0;item[i].pop_back()){
          A+=item[i].back();
        }
      }else{
        wait.push_back(item[i].back());
      }
    }
